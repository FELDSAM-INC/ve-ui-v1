...
task 'assets:precompile:css' => 'environment' do
  if ENV["DONT_PRECOMPILE_CSS"] == "1"
    ...
    STDERR.puts "Compiling css for prod"
    [:desktop].each do |target|
      STDERR.puts "target: #{target} #{NilavuStylesheets.compile(target)}"
    end
...
def compress_node(from,to)
  ...
  cmd = "uglifyjs -c -m -o '#{to_path}' --source-map '#{assets_path}/#{from}'"
